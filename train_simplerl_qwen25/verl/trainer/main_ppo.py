...
from verl.utils.reward_score import medreasoning
...
# Note: To use the new grpo_weighted advantage methods, set the following in your config:
# For addition-based weighting: config.algorithm.adv_estimator = "grpo_weighted_add"
# For multiplication-based weighting: config.algorithm.adv_estimator = "grpo_weighted_mul"
# config.algorithm.enable_solution_classification = True  # Set to False to disable GPT-4 classification
...
def _default_compute_score(data_source, solution_str, ground_truth):
    ...
    elif data_source == 'textbookreasoning_physics':
        return megascience.compute_score(solution_str, ground_truth)
    elif data_source == 'MedCaseReasoning' or 'medcase' in str(data_source).lower():
        # Align with MedCaseReasoning: use LLM-as-judge by default
        score = medcase_reasoning.compute_score(solution_str, ground_truth)
        return {'score': float(score), 'correctness': float(score)}
    elif 'medreason' in str(data_source).lower():
        # MedReason multiple-choice questions: deterministic letter matching
        return medreasoning.compute_score(solution_str, ground_truth)
    else:
