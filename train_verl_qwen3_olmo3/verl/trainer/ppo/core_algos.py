...
def compute_grpo_weighted_advantage_add(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    """
    GRPO with diversity weighting based on solution grouping (ADDITION).

    This mirrors the simpleRL-reason-mine implementation:
      1) Compute standard GRPO scalar advantages per response (per prompt group).
      2) For each prompt group, compute frequency of each "solution group".
      3) Define a diversity weight w_i = 1 / (freq_i ** weight_power).
      4) Final scalar advantage = a_i^GRPO + w_i, then broadcast to tokens.

    Args:
        token_level_rewards: (bs, response_len)
        response_mask: (bs, response_len)
        index: np.ndarray of shape (bs,) – prompt ids for GRPO grouping
        group_index: tensor of shape (bs,) – solution-group ids within each prompt
        weight_power: exponent in the diversity weight
        epsilon: numerical stability

    Returns:
        advantages: (bs, response_len)
        returns: (bs, response_len)
        components: dict with token-level and scalar GRPO/diversity parts
    """
    ...
    with torch.no_grad():
        ...
        # --- 1) standard GRPO over prompt groups ---
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
        for idx_key, score_list in id2score.items():
            if len(score_list) == 1:
                id2mean[idx_key] = torch.tensor(0.0, device=scores.device, dtype=scores.dtype)
                id2std[idx_key] = torch.tensor(1.0, device=scores.device, dtype=scores.dtype)
            elif len(score_list) > 1:
                scores_tensor = torch.stack(score_list)
                id2mean[idx_key] = torch.mean(scores_tensor)
                id2std[idx_key] = torch.std(scores_tensor)
            else:
                raise ValueError(f"no score in prompt index: {idx_key}")

        grpo_advantages = torch.zeros_like(scores)
        for i in range(bsz):
            key = index[i]
            grpo_advantages[i] = (scores[i] - id2mean[key]) / (id2std[key] + epsilon)

        # --- 2) diversity weights based on solution grouping within each prompt ---
        group_freq: dict[Any, dict[int, int]] = defaultdict(lambda: defaultdict(int))
        for i in range(bsz):
            prompt_idx = index[i]
            group_idx = int(group_index[i].item())
        ...
        for i in range(bsz):
            prompt_idx = index[i]
            group_idx = int(group_index[i].item())
            freq = max(group_freq[prompt_idx][group_idx], 1)
            weights[i] = 1.0 / (float(freq) ** float(weight_power))

        # --- 3) combine (ADDITION) and broadcast to token-level ---
        combined_scalars = grpo_advantages + weights
        combined_advantages = combined_scalars.unsqueeze(-1) * response_mask

        grpo_advantages_expanded = grpo_advantages.unsqueeze(-1) * response_mask
        weights_expanded = weights.unsqueeze(-1) * response_mask

    components = {
        "grpo_advantages": grpo_advantages_expanded,
        "diversity_weights": weights_expanded,
        "grpo_advantages_scalar": grpo_advantages,
        "diversity_weights_scalar": weights,
    }
    return combined_advantages, combined_advantages, components


def compute_grpo_weighted_advantage_mul(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    group_index: torch.Tensor,
    ...
    """
    GRPO with diversity weighting based on solution grouping (MULTIPLICATION).

    Same as compute_grpo_weighted_advantage_add, except:
        final_scalar = a_i^GRPO * w_i

    This is the variant used in the original creativity experiments.
    """
    scores = token_level_rewards.sum(dim=-1)  # (bs,)
    ...
    with torch.no_grad():
        ...
        # --- 3) combine (MULTIPLICATION) and broadcast to token-level ---
        combined_scalars = grpo_advantages * weights
        combined_advantages = combined_scalars.unsqueeze(-1) * response_mask
