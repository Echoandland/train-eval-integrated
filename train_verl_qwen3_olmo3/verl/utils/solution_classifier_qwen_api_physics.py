...
class Entropy_Calculation_Classify_By_Qwen:
    ...
    def analyze_solutions_with_qwen(self, n_solutions):
        try:
            ...
            # )
            user_input = (
                "Here are several solutions to the same *physics* question:" + str_solutions +
                "Please analyze and determine how these solutions can be grouped based on the high-level physical principles or modeling frameworks they use. "
                "Your classification criteria must remain strictly high-level. Place solutions in different categories only when their overarching strategies are completely distinct; "
                "differences limited to sub-steps, choice of coordinates, or algebraic rearrangements do not count as high-level distinctions. "
                "Before you begin grouping, clearly state the classification criteria you will follow. "
                "In your response, focus on explaining your reasoning and clearly state which solution indices should be grouped together. "
                "Note that if all solutions use entirely different approaches, each should be placed in its own distinct group. "
                "In your grouping, each solution should be assigned to exactly one of the groups. Make sure to carefully check the total number of solutions. "
                ...
                "Group 1 – Energy / Work–Energy method\n"
                "• Solution 1\n• Solution 2\n\n"
                "Both derive the result by writing ΔK = W_nonconservative + ΔU (or mechanical-energy conservation when appropriate). "
                "They compute speeds or heights from energy balance without integrating equations of motion or introducing generalized coordinates.\n\n"
                "Group 2 – Newton’s second law (force balance + kinematics)\n"
                "• Solution 3\n\n"
                "This approach draws a free-body diagram, resolves forces (e.g., along an incline), writes ma = ΣF, and integrates a(t) to get v or x; "
                "it does not use energy balance as the primary tool.\n\n"
                "Group 3 – Lagrangian formulation (generalized coordinates, constraints)\n"
                "• Solution 4\n\n"
                "This solution sets up L = T − V with a generalized coordinate, applies the Euler–Lagrange equation (optionally with Rayleigh dissipation or constraints). "
                "Conceptually distinct from both the direct force-balance method and the energy accounting used in Group 1.\n\n"
                "Thus every solution belongs to exactly one of three distinct groups:\n"
                "• Group 1: 1, 2\n"
                "• Group 2: 3\n"
                "• Group 3: 4"
